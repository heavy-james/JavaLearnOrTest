...
public class RegularExpressionTest extends TestCase{
	...
	public static final String REGULAR_ENUM_NAMES = "\\(\\s*" + REGULAR_NAME
			+ "\\s*(\\|\\s*" + REGULAR_NAME + "\\s*)*\\s*\\)";
	...
	private static final String REGULAR_ATTR_ENUM_VALUE = "("
			+ REGULAR_ENUM_NAMES + "|" + REGULAR_ENTITY_REFERENCE 
	...
	private static final String REGULAR_ATTRI_NAME_VALUE_EXISTENCE = "\\s*"
			+ REGULAR_ATTRI_NAME + "\\s+" + REGULAR_ATTR_ENUM_VALUE + "\\s+"
			+ REGULAR_ATTR_ENUM_EXISTENCE + "\\s*";


	private static final String REGULAR_ATTR_CONTENT = "(\\s*"
			+ REGULAR_ATTRI_NAME_VALUE_EXISTENCE + "|" + REGULAR_ENTITY_REFERENCE
			+ "\\s*)";
	...
	public void run() {
		...
		//boolean res = Pattern.matches(REGULAR_ENTITY_REFERENCE, " %print-object;");
		
		
		String content = "<!ATTLIST staff-details" +
		...
		if (content != null && content.startsWith("<!ATTLIST")
				&& content.endsWith(">")) {
			content = content.substring("<!ATTLIST".length(), content.length()
					- ">".length());
			...
			if (attriStrs != null) {
				Pattern attriDefPattern = Pattern
						.compile(REGULAR_ATTRI_NAME_VALUE_EXISTENCE);
				Pattern entityRefPattern = Pattern
						.compile(REGULAR_ENTITY_REFERENCE);
